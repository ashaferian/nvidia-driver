...
void   nvidia_pci_check_config_space  (nvidia_stack_t *, device_t dev, BOOL, BOOL, BOOL);
...
#define NV_PCI_CHECK_CONFIG_SPACE(sp,nv,cb,as,mb)                       \
    {                                                                   \
        struct nvidia_softc *__sc;                                      \
        if ((((nv)->flags & NV_FLAG_CONTROL) == 0) &&                   \
            (((nv)->flags & NV_FLAG_SKIP_CFG_CHECK) == 0)) {            \
            __sc = (nv)->os_state;                                      \
            nvidia_pci_check_config_space(sp, __sc->dev, cb, as, mb);   \
        }                                                               \
    }
...
NvS32  nv_alloc_contig_pages (nv_state_t *, NvU32, NvU32, NvBool, NvU64 *, void **);
NvS32  nv_free_contig_pages  (nv_state_t *, void *);
NvS32  nv_alloc_system_pages (nv_state_t *, NvU32, NvU32, NvBool, NvU64 *, void **);
NvS32  nv_free_system_pages  (nv_state_t *, void *);
