...
int nvidia_sysctl_gpu_model(SYSCTL_HANDLER_ARGS)
{
    ...
    if (rm_get_device_name(sp, nv, nv->pci_info.device_id, subvendor, subdevice,
                           NV_DEVICE_NAME_LENGTH, model_name) != NV_OK) {
...
int nvidia_sysctl_gpu_uuid(SYSCTL_HANDLER_ARGS)
{
    ...
    if (rm_get_gpu_uuid(sp, nv, &uuid, &uuid_strlen) != NV_OK)
        return EIO;

    NV_UMA_ZONE_FREE_STACK(sp);

    ret = SYSCTL_OUT(req, (char *)uuid, uuid_strlen);
    os_free_mem(uuid);
...
int nvidia_sysctl_gpu_vbios(SYSCTL_HANDLER_ARGS)
{
    ...
    NvU32 vbios[5];
    NvU8  vbios_version[16];

    NV_UMA_ZONE_ALLOC_STACK(sp);
    if (sp == NULL)
        return ENOMEM;

    if (rm_get_vbios_version(sp, nv, &vbios[0], &vbios[1], &vbios[2],
                &vbios[3], &vbios[4]) != NV_OK) {
        /*
